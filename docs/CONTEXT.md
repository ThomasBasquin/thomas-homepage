> **Note d'origine** : ce document décrit le projet source [AbhishekBadar/portfolio](https://github.com/AbhishekBadar/portfolio) dont ce site est adapté. L'architecture reste valable ; le contenu (profil, projets, CV) a été remplacé par celui de Thomas Basquin dans `src/lib/data.ts`.
